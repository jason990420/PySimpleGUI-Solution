...
from token import NAME, STRING, NEWLINE, INDENT, DEDENT, OP, COMMENT, NL
from tokenize import tokenize

import PySimpleGUI as sg

"""
Python 3.7+ required for update token library
"""
...
class GUI():
    ...
    def Tree(self, key):
        ...
        show_expanded=False, col0_width=40, auto_size_columns=False,
        ...
        font=self.font2, justification='left', expand_y=True,
        vertical_scroll_only=False, key=key)
    ...
    def Multiline(self, key):
        return sg.Multiline(default_text='', size=self.line_width,
            enable_events=False, pad=((0, 5), 0), focus=False, key=key,
            do_not_clear=True, disabled=True, border_width=0, font=self.font1,
            text_color='white', background_color='blue', expand_x=True, expand_y=True)
...
while True:
    ...
    if event in [None, 'Quit']:
        ...
        func[event](values)
    elif event.endswith("+WHEEL"):
        update_wheel(event, g.window)
    key = g.where()
